...
def main():
    ...
    if get_pic:
        image = tao_anh()
        st.image(image, width=460, caption='Ảnh chữ số viết tay')
    with col2:
        get_dect = st.button("Nhận dạng")
    if get_dect:
        st.image('./images/digit_random.jpg', width=460,
                 caption='Kết quả nhận dạng chữ số')
        image = Image.open('./images/digit_random.jpg')
        prediction = nhan_dang(image)
        results = np.argmax(prediction, axis=1)
        columns = st.columns(15)
        for i in range(len(results)):
            with columns[i % 15]:
                st.write(results[i], font_size=80, bg_color='blue',
                         text_color='black', style='text-align: center')
